...
# 37 "mcc_generated_files/bootloader/library/core/bl_memory.h" 2
...
# 31 "mcc_generated_files/bootloader/library/core/bl_memory.c" 2
...
static flash_data_t writeBuffer[(256U)];
...
# 159 "mcc_generated_files/bootloader/library/core/bl_memory.c"
bl_mem_result_t BL_FlashRead(flash_address_t address, flash_data_t * buffer, size_t length)
{
    ...
    else if (currentAddress > (0x020000U))
    {
        result = BL_MEM_INVALID_ARG;
    }
    else if ((length <= (size_t) 0) || (length > (size_t) (256U)) || ((currentAddress + length - (size_t)1) > ((0x020000U))))
    {
        result = BL_MEM_INVALID_ARG;
    }
    else
...
bl_mem_result_t BL_FlashWrite(flash_address_t address, flash_data_t * buffer, size_t length)
{
    ...
    else if (address > (0x020000U))
    {
        result = BL_MEM_INVALID_ARG;
    }
    else if ((length <= (size_t) 0) || (length > (size_t) (256U)) || ((address + length - (size_t)1) > ((0x020000U))))
    {
        result = BL_MEM_INVALID_ARG;
    }
    else
    {
        ...
        if( BL_MEM_PASS == result)
        {
            ...
            else
            {
                ...
                uint16_t postReadByteCount = (uint16_t) ((256U) - loadedCount);
                ...
                if (BL_MEM_PASS == result)
                {
                    ...
                    if(NVM_OK == FLASH_PageErase(pageStartAddress))
                    {
                        result = BL_MEM_PASS;
                    }
                    else
...
bl_mem_result_t BL_FlashCopy(flash_address_t srcAddress, flash_address_t destAddress, size_t length)
{
    ...
    uint32_t flashEndAddress = (0x020000U) + (uint32_t) 1;


    if (
            (srcAddress > (0x020000U)) ||
            (destAddress > (0x020000U))
            )
    {
        result = BL_MEM_INVALID_ARG;
    }
